...
class two_dimension_Coulomb_SBM_solution():
    def __init__(self,T:float,L:float,n:int,Lambda:float,gamma:float,epsilon:float,sigma:float=0.7):
        ...
        self.reference_solution = np.load(os.path.join(os.getcwd(),\
            'RBM_reference_solution\\T=200_L=5_dt=0.02_n=200_epsilon=0.04_every_second_RBM_solution.npy'))
...
def plot(T:float,V:np.ndarray,L:float,n:int,dt:float,address:str)->None:
    ...
    if T==0:
        ...
        V_reference_solution = np.load(os.path.join(os.getcwd(),\
            'RBM_reference_solution\\T=200_L=5_dt=0.02_n=200_epsilon=0.04_every_second_RBM_solution.npy'))[int(T-1),:].reshape(-1,d)
...
dt=0.1
Dt=1
point=np.load(os.path.join(os.getcwd(),'initial_distribution_sampling\\100000_points_2d-Coulomb.npy'))[:10000]
N=point.shape[0]
DT=int(Dt/dt)
epsilon=0.01
Lambda=1/16
Lambda=Lambda*2
gamma=-3
address=os.path.join(os.getcwd(),'2d_Coulomb','SBM',f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}')
...
def save_and_draw(T:float,v:float,n:int,point:np.ndarray,epsilon:float,dt:float)->None:
    ...
    np.save(os.path.join(address,f'relative-entropy(SBM_vs_reference)'),relative_entropy)
    ...
    plt.savefig(os.path.join(address,f'relative-entropy(SBM_vs_reference).png'),dpi=160)
    plt.close()
    # plt.show()
