...
dt=0.1
Dt=1
point=np.load(address/'data'/'initial_distribution_sampling'/'100000_points_2d-Coulomb.npy')[:10000]
N=point.shape[0]
DT=int(Dt/dt)
epsilon=0.01
Lambda=1/16
Lambda=Lambda*2
gamma=-3
address_save=address/'result'/'2d_Coulomb'/'EM'/f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}'
...
def save_and_draw(T:float,v:float,n:int,point:np.ndarray,epsilon:float,dt:float,path:Path)->None:
    ...
    Y = two_dimension_Coulomb_EM_solution(T,v,n,Lambda,gamma,epsilon)
