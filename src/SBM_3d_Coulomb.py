...
class three_dimension_Coulomb_SBM_solution():
    def __init__(self,T:float,L:float,n:int,Lambda:float,gamma:float,epsilon:float,sigma:float=0.7):
        ...
        self.reference_solution = np.load(os.path.join(os.getcwd(),\
            'data','RBM_reference_solution\\T=200_L=2_dt=0.05_n=40_epsilon=0.01_every_second_RBM_solution.npy'))
...
dt=0.1
Dt=1
point=np.load(os.path.join(os.getcwd(),'data','initial_distribution_sampling\\500000_points_3d-Coulomb.npy'))[:50000]
N=point.shape[0]
DT=int(Dt/dt)
epsilon=0.01
Lambda=1/(4*np.pi)
...
address=os.path.join(os.getcwd(),'3d_Coulomb','SBM',f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}')
