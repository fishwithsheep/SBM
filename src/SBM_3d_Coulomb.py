from function import mollifier, random_group, sphere_brown
    ...
    def solve(self,dt:float,V:np.ndarray,path:Path)->tuple:
        ...
        for t in range(int(self.T / dt)):
            ...
            for i in range(int(length/2)):
                ...
                #* If |z| is to small, then in ((2 * self.Lambda ** 0.5 * np.linalg.norm(z) ** (self.gamma / 2)) ** 2 * dt) will be very 
                #* large, hence in sphere_brown beta will be very huge and in 'eta = beta / (np.exp(beta) - 1)' will encounter overflow 
                #* in exp. Either can limit the up bound of |z| or let eta=0 when such case. Here we use the first method and choose the
                #* upbound as 0.25(the accuracy is sufficient), and by different experiment we verify that the upbound taking by 0.05 is
                #* still possible, while taking by 0.01 will encounter overflow in exp.
...
dt=0.1
Dt=1
point=np.load(address/'data'/'initial_distribution_sampling'/'500000_points_3d-Coulomb.npy')[:50000]
N=point.shape[0]
DT=int(Dt/dt)
epsilon=0.01
Lambda=1/(4*np.pi)
...
address_save=address/'result'/'3d_Coulomb'/'SBM'/f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}'
...
def save_and_draw(T:float,v:float,n:int,point:np.ndarray,epsilon:float,dt:float,path:Path)->None:
    ...
    Y = three_dimension_Coulomb_SBM_solution(T,v,n,Lambda,gamma,epsilon)
