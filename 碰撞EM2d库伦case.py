...
class two_dimension_Coulomb_EM_solution():
    def __init__(self,T,L,n,Lambda,gamma,epsilon,sigma=0.7):
        ...
        self.reference_solution = np.load('RBM参考解\\T=200_L=5_dt=0.02_n=200_yipuxilong=0.04_every_second_RBM_solution.npy')
    ...
    def exact_solution(self, T):
        '输出T时刻、[-L,L]^2速度空间格点中心处的点(n^2,1)质量矩阵(参考解)'
    ...
    def compute_parameter(self,V,num,T):
        ...
        z_initial = 1 / (4 * np.pi) * (
                0.4 * np.exp(-np.linalg.norm(self.V__ - Z1, axis=-1) ** 2 / 2) + 1.6 * np.exp(
            -np.linalg.norm(self.V__ - Z2, axis=-1) ** 2 / 2)).reshape(-1, 1) * self.h0 ** 2
    ...
    def solve(self,dt,V):
        ...
        for t in range(int(self.T / dt)):
            ...
            for i in range(int(length/2)):
                ...
                V2=V[p2].reshape(1, -1)
                z=V1-V2
                brown = np.random.normal(loc=0, scale=dt ** 0.5, size=[2, 1])
                z__ = -self.Lambda * z * dt *np.linalg.norm(z)**self.gamma + np.matmul(square_collision_kernel_A(Lambda=self.Lambda, d=2, x=z, gamma=self.gamma),
                    brown).T
            ...
            if t % int(1 / dt) == int(1 / dt) - 1:
                np.save(f'T={(t + 1) * dt}_dt={dt}_n={self.n}_N={length}_epsilon={self.epsilon}_2d-Coulomb_EM_velocity', V)
...
def plot(T,V,n,v,dt):
    'T为画图时刻，V为(length,1)速度矩阵，n为[-v,v]取点数,n0为参考解[-v,v]取点数,v为画图最后不要的点的分量上界'
    length = V.shape[0]
    ...
    #参考解计算格点上的解
    h0 = (2 * v) / n0
    a0 = np.linspace(-v + h0 / 2, v - h0 / 2, n0, dtype=float)
    VX0, VY0 = np.meshgrid(a0, a0)
    V0 = np.stack((VX0, VY0), axis=2).reshape([-1, 2])
    if T==0:
        a__ = np.linspace(-v + h0 / 2, v - h0 / 2, n0, dtype=float)
        vx__, vy__ = np.meshgrid(a__, a__)
        V__ = np.stack((vx__, vy__), axis=2).reshape(-1, 2)
        V_reference_solution = V__
    else:
        V_reference_solution = np.load('RBM参考解\\T=200_L=5_dt=0.02_n=200_yipuxilong=0.04_every_second_RBM_solution.npy')[int(T-1),:].reshape(-1,2)
    Z1 = np.concatenate((-2 * np.ones(shape=[n0 ** 2, 1]), 1 * np.ones(shape=[n0 ** 2, 1])), axis=1)
    Z2 = np.concatenate((1 * np.ones(shape=[n0 ** 2, 1]), -1 * np.ones(shape=[n0 ** 2, 1])), axis=1)
    ...
    p20=np.linspace(-v, v, n0, dtype=float).reshape(-1,1)
    ...
    for i in range(n):
        x = np.repeat(V0[i].reshape(1, -1), n0 ** 2, axis=0) - V_reference_solution
        Z_[i] = np.sum(np.multiply(mollifier(d=2, x=x, epsilon=epsilon).reshape([-1, 1]), z_initial), axis=0)
    Z_ = Z_.reshape(-1, 1)
    ...
    plt.ylabel('f')
    plt.legend()
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_cross-section.png',dpi=160)
    plt.close()
    # plt.show()

    #3d figure
    Z_ = np.zeros(shape=[n ** 2, 1])
    for i in range(n ** 2):
        x = np.repeat(V_[i].reshape(1, -1), n0 ** 2, axis=0) - V_reference_solution
        Z_[i] = np.sum(np.multiply(mollifier(d=2, x=x, epsilon=epsilon).reshape([-1, 1]), z_initial), axis=0)
    Z_ = Z_.reshape(-1, n)
    ...
    ay.set_title('EM')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_3d-graph.png',dpi=160)
    plt.close()
    # plt.show()
...
dt=0.1
Dt=1
point=np.load('随机取样的点\\100000_points_2d-Coulomb.npy')[:10000]
N=point.shape[0]
DT=int(Dt/dt)
epsilon=0.01
Lambda=1/16
...
def save_and_draw(T,v,n,point,N,epsilon,dt):
    ...
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_grid-mass', mass)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_grid-momentum', momentum)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_grid-energy', energy1)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_particle-energy', energy2)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_relative-L2-error', relative_L2_error)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_entropy', entropy)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_relative-entropy(EM_vs_reference)',relative_entropy)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_total-time', Time)
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_grid-mass.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.ylabel('momentum')
    plt.title('momentum')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2dCoulomb_EM_grid-momentum.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, energy1)
    plt.xlabel("T")
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_grid-energy.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, energy2)
    plt.xlabel("T")
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_particle-energy.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_relative-L2-error.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, entropy)
    plt.xlabel("T")
    plt.ylabel('entropy')
    plt.title('entropy')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_entropy.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Coulomb_EM_relative-entropy(EM_vs_reference).png',
                dpi=160)
    plt.close()
    # plt.show()
