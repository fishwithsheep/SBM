...
class three_dimension_Maxwell_EM_solution():
    def __init__(self,T,L,n,Lambda,gamma,epsilon,sigma=0.7):
        ...
        Q = np.mgrid[-self.L + self.h / 2: self.L + self.h / 2: self.h,
            -self.L + self.h / 2: self.L + self.h / 2: self.h,
            -self.L + self.h / 2: self.L + self.h / 2: self.h]
        self.vx = Q[0].reshape([-1, 1])
        self.vy = Q[1].reshape([-1, 1])
        self.vz = Q[2].reshape([-1, 1])
    ...
    def compute_parameter(self,V,num,T):
        'num是粒子总数,T是时间，计算格点质量分布时丢掉距离≥0.7的点，以便加速计算误差，但不会影响EM精度'
    ...
    def solve(self,dt,V):
        ...
        for t in range(int(self.T / dt)):
            ...
            if t % int(1 / dt) == int(1 / dt) - 1:
                np.save(f'T={(t + 1) * dt}_dt={dt}_n={self.n}_N={length}_epsilon={self.epsilon}_3d-Maxwell_EM_velocity', V)
...
def save_and_draw(T,v,n,point,N,epsilon,dt):
    ...
    Y = three_dimension_Maxwell_EM_solution(T=T, L=v, n=n, Lambda=Lambda, gamma=gamma, epsilon=epsilon)
    V, mass, momentum, energy1, energy2, relative_L2_error, entropy, relative_entropy, Time = Y.solve(dt=dt, V=point)

    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_grid-mass', mass)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_grid-momentum', momentum)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_grid-energy', energy1)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_particle-energy', energy2)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_relative-L2-error', relative_L2_error)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_entropy', entropy)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_relative-entropy(EM_vs_analytic)',relative_entropy)
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_grid-mass.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.ylabel('momentum')
    plt.title('momentum')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_grid-momentum.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, energy1)
    plt.xlabel("T")
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_grid-energy.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, energy2)
    plt.xlabel("T")
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_particle-energy.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_relative-L2-error.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, entropy)
    plt.xlabel("T")
    plt.ylabel('entropy')
    plt.title('entropy')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_entropy.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_3d-Maxwell_EM_relative-entropy(EM_vs_analytic).png',
                dpi=160)
    plt.close()
    # plt.show()
