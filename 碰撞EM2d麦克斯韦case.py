...
class two_dimension_Maxwell_EM_solution():
    def __init__(self,T,L,n,Lambda,gamma,epsilon,sigma=0.7):
        ...
        #epsilon为磨光核参数，sigma为磨光核计算时距离阈值(减少计算时间)，self.V_为(n^2,2)速度格点中心矩阵
    ...
    def compute_parameter(self,V,num,T):
        'num是粒子总数，T是时间，epsilon为磨光常数，计算格点质量分布时丢掉距离≥0.7的点，以便加速计算误差，但不会影响EM精度'
    ...
    def solve(self,dt,V):
        ...
        for t in range(int(self.T / dt)):
            ...
            if t % int(1/dt) == int(1/dt)-1:
                np.save(f'T={(t+1)*dt}_dt={dt}_n={self.n}_N={length}_epsilon={self.epsilon}_2d-Maxwell_EM_velocity', V)
...
def plot(T,V,n,v,dt):
    ...
    plt.ylabel('f')
    plt.legend()
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_cross-section.png',dpi=160)
    plt.close()
    # plt.show()
    ...
    ay.set_title('EM')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_3d-graph.png',dpi=160)
    plt.close()
    # plt.show()
...
def save_and_draw(T,v,n,point,N,epsilon,dt):
    ...
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_grid-mass', mass)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_grid-momentum', momentum)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_grid-energy', energy1)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_particle-energy', energy2)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_relative-L2-error',relative_L2_error)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_entropy', entropy)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_relative-entropy(EM_vs_analytic)',relative_entropy)
    np.save(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_total-time', Time)
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_grid-mass.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.ylabel('momentum')
    plt.title('momentum')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_grid-momentum.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, energy1)
    plt.xlabel("T")
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_grid-energy.png', dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, energy2)
    plt.xlabel("T")
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_particle-energy.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_relative-L2-error.png',dpi=160)
    plt.close()
    # plt.show()
    plt.plot(t, entropy)
    plt.xlabel("T")
    plt.ylabel('entropy')
    plt.title('entropy')
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_entropy.png', dpi=160)
    plt.close()
    # plt.show()
    ...
    plt.savefig(f'T={T}_dt={dt}_n={n}_N={N}_epsilon={epsilon}_2d-Maxwell_EM_relative-entropy(EM_vs_analytic).png',dpi=160)
    plt.close()
    # plt.show()
